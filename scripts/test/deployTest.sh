...
sed \
  ...
  -e "s|^sleeper.optional.stacks=.*|sleeper.optional.stacks=CompactionStack,GarbageCollectorStack,PartitionSplittingStack,QueryStack,SystemTestStack,IngestStack|" \
...
echo "-------------------------------------------------------------------------------"
echo "Finished deploy & test"
echo "-------------------------------------------------------------------------------"
echo "Started deploy & test at $(recorded_time_str "$START_TIME")"
echo "Configuring deployment finished at $(recorded_time_str "$END_CONFIGURE_DEPLOYMENT_TIME"), took $(elapsed_time_str "$START_TIME" "$END_CONFIGURE_DEPLOYMENT_TIME")"
echo "Stack deployment finished at $(recorded_time_str "$END_STACK_DEPLOYMENT_TIME"), took $(elapsed_time_str "$END_CONFIGURE_DEPLOYMENT_TIME" "$END_STACK_DEPLOYMENT_TIME")"
...
echo "Overall, deploy & test took $(elapsed_time_str "$START_TIME" "$FINISH_TIME")"
