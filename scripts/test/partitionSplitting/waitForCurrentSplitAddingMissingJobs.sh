...
java -cp "${SYSTEM_TEST_JAR}" \
 sleeper.systemtest.drivers.compaction.WaitForCurrentSplitAddingMissingJobs "$INSTANCE_ID" "$TABLE_NAME"

FINISH_TIME=$(record_time)
echo "-------------------------------------------------------------------------------"
echo "Finished waiting for current partition split"
echo "-------------------------------------------------------------------------------"
echo "Started at $(recorded_time_str "$START_TIME")"
echo "Splitting finished at $(recorded_time_str "$FINISH_TIME"), took $(elapsed_time_str "$START_TIME" "$FINISH_TIME")"
