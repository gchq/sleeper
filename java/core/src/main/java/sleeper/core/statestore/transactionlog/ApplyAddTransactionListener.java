...
/**
 * Listens to see the state before a transaction is applied to the local state, after it is added to the log. This is
 * when the transaction is in the log, but before it is applied locally.
 *
 * @param <S> the type of state to listen on
 */
@FunctionalInterface
public interface ApplyAddTransactionListener<S> {
     ...
     * @return     the listener
     */
    static <S> ApplyAddTransactionListener<S> none() {
        return (number, state) -> {
        };
    }
}
