...
class IngestRecordsIT extends IngestRecordsTestBase {
    ...
    @Nested
    @DisplayName("Using one reference per leaf file writing strategy")
    class OneReferencePerLeafStrategy {
        ...
        void shouldWriteRecordsSplitByPartitionByteArrayKey() throws Exception {
            ...
            AssertQuantiles.forSketch(getSketches(schema, firstReference.getFilename()).getQuantilesSketch("key"))
                    ...
                    .quantile(0.0, wrap(new byte[]{1, 1})).quantile(0.1, wrap(new byte[]{1, 1}))
                    .quantile(0.2, wrap(new byte[]{1, 1})).quantile(0.3, wrap(new byte[]{1, 1}))
                    .quantile(0.4, wrap(new byte[]{2, 2})).quantile(0.5, wrap(new byte[]{2, 2}))
                    .quantile(0.6, wrap(new byte[]{2, 2})).quantile(0.7, wrap(new byte[]{64, 65}))
                    .quantile(0.8, wrap(new byte[]{64, 65})).quantile(0.9, wrap(new byte[]{64, 65})).verify();
        }
        ...
        private FileReference accurateSplitFileReference(String partitionId, long numberOfRecords) {
            return IngestCoordinatorTestHelper.accurateSplitFileReference("rootFile", partitionId, numberOfRecords, Instant.MIN);
        }
    }
}
