 ...
 * </li>
 * <li>
 * When the {@link RecordBatch} is full, the data is retrieved from the {@link RecordBatch} as {@link Row}
 * objects,
 * in sorted order.
 * </li>
 * <li>
 * The sorted rows are passed to an {@link IngesterIntoPartitions} object, which uses
 * {@link sleeper.ingest.runner.impl.partitionfilewriter.PartitionFileWriter} objects to create the partition files in
 * the
...
public class IngestCoordinator<INCOMINGDATATYPE> implements AutoCloseable {
    ...
    private void initiateIngestIfNecessary(boolean isClosing) throws StateStoreException, IteratorCreationException, IOException {
        ...
        if (isClosing || currentRecordBatch.isFull()) {
            ...
            try (CloseableIterator<Row> orderedRecordIteratorFromBatch = currentRecordBatch.createOrderedRecordIterator();
