...
public interface compactionProperties {
    UserDefinedInstanceProperty ECR_COMPACTION_REPO = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.compaction.repo")
    ...
    UserDefinedInstanceProperty COMPACTION_QUEUE_VISIBILITY_TIMEOUT_IN_SECONDS = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.compaction.queue.visibility.timeout.seconds")
            .description("The visibility timeout for the queue of compaction jobs.")
            .defaultValue("900")
            .propertyGroup(InstancePropertyGroup.COMPACTION)
            .runCDKDeployWhenChanged(true).build();
    UserDefinedInstanceProperty COMPACTION_KEEP_ALIVE_PERIOD_IN_SECONDS = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.compaction.keepalive.period.seconds")
    ...
    UserDefinedInstanceProperty COMPACTION_JOB_CREATION_LAMBDA_PERIOD_IN_MINUTES = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.compaction.job.creation.period.minutes")
            .description("The rate at which the compaction job creation lambda runs (in minutes, must be >=1).")
            .defaultValue("1")
            .validationPredicate(Utils::isPositiveInteger)
            .propertyGroup(InstancePropertyGroup.COMPACTION)
            .runCDKDeployWhenChanged(true).build();
    UserDefinedInstanceProperty COMPACTION_JOB_CREATION_LAMBDA_MEMORY_IN_MB = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.compaction.job.creation.memory")
            .description("The amount of memory for the lambda that creates compaction jobs.")
            .defaultValue("1024")
            .propertyGroup(InstancePropertyGroup.COMPACTION)
            .runCDKDeployWhenChanged(true).build();
    UserDefinedInstanceProperty COMPACTION_JOB_CREATION_LAMBDA_TIMEOUT_IN_SECONDS = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.compaction.job.creation.timeout.seconds")
            ...
            .propertyGroup(InstancePropertyGroup.COMPACTION)
            .runCDKDeployWhenChanged(true).build();
    UserDefinedInstanceProperty MAXIMUM_CONCURRENT_COMPACTION_TASKS = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.compaction.max.concurrent.tasks")
            .description("The maximum number of concurrent compaction tasks to run.")
            .defaultValue("300")
            .propertyGroup(InstancePropertyGroup.COMPACTION).build();
    UserDefinedInstanceProperty COMPACTION_TASK_CREATION_PERIOD_IN_MINUTES = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.compaction.task.creation.period.minutes")
            .description("The rate at which a check to see if compaction ECS tasks need to be created is made (in minutes, must be >= 1).")
            .defaultValue("1")
            .validationPredicate(Utils::isPositiveInteger)
            .propertyGroup(InstancePropertyGroup.COMPACTION)
            .runCDKDeployWhenChanged(true).build();
    UserDefinedInstanceProperty COMPACTION_TASK_CPU_ARCHITECTURE = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.compaction.task.cpu.architecture")
            ...
            .propertyGroup(InstancePropertyGroup.COMPACTION)
            .runCDKDeployWhenChanged(true).build();
    UserDefinedInstanceProperty COMPACTION_TASK_ARM_CPU = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.compaction.task.arm.cpu")
            ...
            .propertyGroup(InstancePropertyGroup.COMPACTION)
            .runCDKDeployWhenChanged(true).build();
    UserDefinedInstanceProperty COMPACTION_TASK_ARM_MEMORY = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.compaction.task.arm.memory")
            .description("The memory for a compaction task using an ARM architecture.\n" +
                    "See https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task-cpu-memory-error.html for valid options.")
            .defaultValue("4096")
            .propertyGroup(InstancePropertyGroup.COMPACTION)
            .runCDKDeployWhenChanged(true).build();
    UserDefinedInstanceProperty COMPACTION_TASK_X86_CPU = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.compaction.task.x86.cpu")
            .description("The CPU for a compaction task using an x86 architecture.\n" +
                    "See https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task-cpu-memory-error.html for valid options.")
            .defaultValue("1024")
            .propertyGroup(InstancePropertyGroup.COMPACTION)
            .runCDKDeployWhenChanged(true).build();
    UserDefinedInstanceProperty COMPACTION_TASK_X86_MEMORY = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.compaction.task.x86.memory")
            .description("The memory for a compaction task using an x86 architecture.\n" +
                    "See https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task-cpu-memory-error.html for valid options.")
            .defaultValue("4096")
            .propertyGroup(InstancePropertyGroup.COMPACTION)
            .runCDKDeployWhenChanged(true).build();
    UserDefinedInstanceProperty COMPACTION_ECS_LAUNCHTYPE = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.compaction.ecs.launch.type")
            ...
            .propertyGroup(InstancePropertyGroup.COMPACTION)
            .build();
    UserDefinedInstanceProperty COMPACTION_EC2_TYPE = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.compaction.ec2.type")
            ...
            .propertyGroup(InstancePropertyGroup.COMPACTION)
            .build();
    UserDefinedInstanceProperty COMPACTION_EC2_POOL_MINIMUM = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.compaction.ec2.pool.minimum")
            ...
            .propertyGroup(InstancePropertyGroup.COMPACTION)
            .build();
    UserDefinedInstanceProperty COMPACTION_EC2_POOL_DESIRED = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.compaction.ec2.pool.desired")
            ...
            .propertyGroup(InstancePropertyGroup.COMPACTION)
            .build();
    UserDefinedInstanceProperty COMPACTION_EC2_POOL_MAXIMUM = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.compaction.ec2.pool.maximum")
            .description("The maximum number of instances for the EC2 cluster (when using EC2-based compactions).")
            .defaultValue("75")
            .validationPredicate(Utils::isNonNegativeInteger)
            .propertyGroup(InstancePropertyGroup.COMPACTION)
            .build();
    UserDefinedInstanceProperty COMPACTION_EC2_ROOT_SIZE = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.compaction.ec2.root.size")
            .description("The size in GiB of the root EBS volume attached to the EC2 instances (when using EC2-based compactions).")
            .defaultValue("50")
            .validationPredicate(Utils::isPositiveInteger)
            .propertyGroup(InstancePropertyGroup.COMPACTION)
            .build();
    UserDefinedInstanceProperty COMPACTION_STATUS_STORE_ENABLED = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.compaction.status.store.enabled")
            .description("Flag to enable/disable storage of tracking information for compaction jobs and tasks.")
            .defaultValue("true")
            .propertyGroup(InstancePropertyGroup.COMPACTION)
            .runCDKDeployWhenChanged(true).build();
    UserDefinedInstanceProperty COMPACTION_JOB_STATUS_TTL_IN_SECONDS = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.compaction.job.status.ttl")
            ...
            .validationPredicate(Utils::isPositiveInteger)
            .propertyGroup(InstancePropertyGroup.COMPACTION).build();
    UserDefinedInstanceProperty COMPACTION_TASK_STATUS_TTL_IN_SECONDS = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.compaction.task.status.ttl")
            .description("The time to live in seconds for compaction task updates in the status store. Default is 1 week.\n" +
                    "The expiry time is fixed when an update is saved to the store, so changing this will only affect new data.")
            .defaultValue("604800") // Default is 1 week
            .validationPredicate(Utils::isPositiveInteger)
            .propertyGroup(InstancePropertyGroup.COMPACTION).build();
    UserDefinedInstanceProperty DEFAULT_COMPACTION_STRATEGY_CLASS = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.default.compaction.strategy.class")
            ...
            .propertyGroup(InstancePropertyGroup.COMPACTION).build();
    UserDefinedInstanceProperty DEFAULT_COMPACTION_FILES_BATCH_SIZE = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.default.compaction.files.batch.size")
            ...
            .defaultValue("11")
            .propertyGroup(InstancePropertyGroup.COMPACTION).build();
    UserDefinedInstanceProperty DEFAULT_SIZERATIO_COMPACTION_STRATEGY_RATIO = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.default.table.compaction.strategy.sizeratio.ratio")
            ...
            .defaultValue("3")
            .propertyGroup(InstancePropertyGroup.COMPACTION).build();
    UserDefinedInstanceProperty DEFAULT_SIZERATIO_COMPACTION_STRATEGY_MAX_CONCURRENT_JOBS_PER_PARTITION = UserDefinedInstanceProperty.Index.propertyBuilder("sleeper.default.table.compaction.strategy.sizeratio.max.concurrent.jobs.per.partition")
