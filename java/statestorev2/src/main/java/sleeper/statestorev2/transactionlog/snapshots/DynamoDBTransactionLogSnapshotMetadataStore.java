...
public class DynamoDBTransactionLogSnapshotMetadataStore {
        ...
        try {
            List<TransactWriteItem> writes = List.of(
                    TransactWriteItem.builder()
                            .put(putNewSnapshot(snapshot, updateTime))
                            .build(),
                    TransactWriteItem.builder()
                            .update(updateLatestSnapshot(snapshot, updateTime))
                            .build());
    ...
    public LatestSnapshots getLatestSnapshots() {
        QueryResponse result = dynamo.query(QueryRequest.builder()
                ...
                .build());
        if (result.count() > 0) {
            return getLatestSnapshotsFromItem(result.items().get(0));
        } else {
    ...
    public Optional<TransactionLogSnapshotMetadata> getLatestSnapshotInRange(SnapshotType type, TransactionLogRange range) {
        if (range.isMaxTransactionBounded()) {
            ...
            return result.items().stream().findFirst().map(item -> getSnapshotFromItem(item));
        } else {
    ...
    public void deleteAllSnapshots() {
        ...
        dynamo.deleteItem(DeleteItemRequest.builder().tableName(latestSnapshotsTable)
    ...
    private Optional<TransactionLogSnapshotMetadata> getLatestSnapshotBefore(SnapshotType type, Instant time) {
        return getSnapshotsBefore(type, time, request -> QueryRequest.builder()
    ...
    private Map<String, AttributeValue> getKeyFromSnapshot(TransactionLogSnapshotMetadata snapshot) {
        return Map.of(TABLE_ID_AND_SNAPSHOT_TYPE, AttributeValue.builder()
                .s(tableAndType(sleeperTableId, snapshot.getType()))
                .build(),
                TRANSACTION_NUMBER, AttributeValue.builder()
                        .n(snapshot.getTransactionNumber() + "")
                        .build());
    }
