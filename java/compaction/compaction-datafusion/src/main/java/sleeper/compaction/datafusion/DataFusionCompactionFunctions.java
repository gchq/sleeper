...
public interface DataFusionCompactionFunctions extends ForeignFunctions {
     ...
     */
    default int compact(FFIContext context, FFIDataFusionCommonConfig input, DataFusionCompactionResult result) {
        return native_compact(context.getForeignContext(), input, result);
    }

    @SuppressWarnings(value = "checkstyle:parametername")
    int native_compact(Pointer context, @In FFIDataFusionCommonConfig input, @Out DataFusionCompactionResult result);
}
