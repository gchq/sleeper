...
template <typename T> struct ScalarVisit : public arrow::ScalarVisitor {
    ScalarVisit(std::vector<T> &ref) : items(ref) {}

    std::vector<T> &items;

    arrow::Status Visit(const arrow::Scalar &s) {
        return arrow::Status::NotImplemented("No valid visitor for", s.type->ToString());
    }
...
template <typename T> std::vector<T> getFirstLast(cudf::column_view const &firstColumn) {
    std::vector<cudf::column_view> sliced =
        cudf::slice(firstColumn, {0, 1, firstColumn.size() - 1, firstColumn.size()});
    ...
    static_cast<void>(arrow::VisitScalarInline(*firstVal.ValueOrDie(), &visitor));
    static_cast<void>(arrow::VisitScalarInline(*secondVal.ValueOrDie(), &visitor));
    return items;
}
