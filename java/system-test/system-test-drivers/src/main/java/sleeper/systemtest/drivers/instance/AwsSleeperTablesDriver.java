...
import sleeper.clients.table.AddTableRestClient;
...
public class AwsSleeperTablesDriver implements SleeperTablesDriver {
    ...
    protected void addTableViaRest(InstanceProperties instanceProperties, TableProperties properties) {
        try (AddTableRestClient client = new AddTableRestClient(instanceProperties, credentialsProvider)) {
            properties.set(TABLE_ID, client.addTable(properties).getTableId());
        }
    }
