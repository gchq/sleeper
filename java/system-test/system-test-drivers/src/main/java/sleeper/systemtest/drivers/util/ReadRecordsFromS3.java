...
public class ReadRecordsFromS3 {
    ...
    public static Stream<SleeperRow> getRecords(String bucketName, String objectKey, Schema schema, Configuration conf) {
        String path = "s3a://" + bucketName + "/" + objectKey;
        List<SleeperRow> records = new ArrayList<>();
        try (ParquetReader<SleeperRow> reader = new ParquetRecordReader.Builder(new Path(path), schema)
                .withConf(conf).build();
