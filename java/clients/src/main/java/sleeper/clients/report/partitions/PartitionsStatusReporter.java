...
public class PartitionsStatusReporter {
    ...
    private static void writeRow(PartitionStatus status, TableRow.Builder builder) {
        ...
        builder.value(ID, partition.getId())
                ...
                .value(APPROX_ROWS, status.getApproxRecords())
                .value(APPROX_ROW_REFERENCED, status.getApproxRecordsReferenced())
                .value(EXACT_ROWS_REFERENCED, status.getExactRecordsReferenced())
