...
public class StandardIngestJobStatusReporterRangeQueryTest {
    ...
    void shouldReportMixedIngestJobs() throws IOException {
        ...
        // When / Then
        assertThat(getStandardReport(JobQuery.Type.RANGE, mixedJobs, 2)).hasToString(
                replaceBracketedJobIds(mixedJobs, example("reports/ingest/job/standard/range/mixedJobs.txt")));
    }
    ...
    void shouldReportMixedUnfinishedIngestJobs() throws IOException {
        ...
        // When / Then
        assertThat(getStandardReport(JobQuery.Type.RANGE, mixedJobs, 2)).hasToString(
                replaceBracketedJobIds(mixedJobs, example("reports/ingest/job/standard/range/unfinishedJobs.txt")));
    }
    ...
    void shouldReportNoIngestJobs() throws IOException {
        ...
        // When / Then
        assertThat(getStandardReport(JobQuery.Type.RANGE, noJobs, 0)).hasToString(
                replaceBracketedJobIds(noJobs, example("reports/ingest/job/standard/range/noJobs.txt")));
    }
}
