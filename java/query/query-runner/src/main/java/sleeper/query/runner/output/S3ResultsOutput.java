...
public class S3ResultsOutput implements ResultsOutput {
    ...
    public ResultsOutputInfo publish(QueryOrLeafPartitionQuery query, CloseableIterator<SleeperRow> results) {
        ...
        long count = 0L;
        try (ParquetWriter<SleeperRow> writer = buildParquetWriter(new Path(outputFile))) {
    ...
    private ParquetWriter<SleeperRow> buildParquetWriter(Path path) throws IOException {
