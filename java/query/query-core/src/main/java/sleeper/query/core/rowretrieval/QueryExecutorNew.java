 ...
 */
public class QueryExecutorNew {

    private final QueryExecutor querySplitter;
    private final LeafPartitionQueryExecutor leafQueryExecutor;

    public QueryExecutorNew(QueryExecutor querySplitter, LeafPartitionQueryExecutor leafQueryExecutor) {
    ...
    public void initIfNeeded(Instant now) throws StateStoreException {
        querySplitter.initIfNeeded(now);
    }
    ...
    public CloseableIterator<Row> execute(Query query) throws QueryException {
        List<LeafPartitionQuery> leafPartitionQueries = querySplitter.splitIntoLeafPartitionQueries(query);
