...
public class SqsQueryProcessor {
    ...
    private SqsQueryProcessor(Builder builder) throws ObjectFactoryException {
        ...
        S3TransferManager s3TransferManager = S3TransferManager.builder().s3Client(S3AsyncClient.create()).build(); //TODO tidy
        stateStoreProvider = StateStoreFactory.createProvider(instanceProperties, builder.s3Client, builder.dynamoClient, s3TransferManager);
    }
