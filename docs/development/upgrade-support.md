...
This includes ECS tasks that may continue to run the old version for hours after the deployment, EMR clusters with jobs
queued with an old version of the jar, and clients that may not be upgraded for some time. For example, compaction tasks
and bulk import jobs will continue to submit state store commits to SQS, and large transactions to S3, with the old
version of the code.
