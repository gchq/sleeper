...
| Ingest Job Status Report      | Lists the ingest and bulk import jobs and the number of messages on the queues | ```./scripts/report/ingestJobStatusReport.sh ${INSTANCE_ID} ${TABLE_NAME} [--report-type <type>] [<query-type-option>]``` | REPORT_TYPE = standard, QUERY_TYPE = prompt. Run with `--help` for the query type options |
