...
the [Development scripts](#development-scripts) section that can display the dependency structure as a graph.
...
When deploying multiple instances (or running multiple system tests), some log groups generated by Lambdas will not be
torn down by AWS CDK. This means that if you want to deploy an instance with the same ID as one you have deployed
previously, then the deployment may fail.

The `CleanUpLogGroups.java` class will clean up any log groups for you that meet the following criteria:

- Log groups that do not belong to a deployed stack and
    - Log groups that do not have a retention period and are older than 30 days or
    - Log groups that are empty.

You can also run this class using a script by running the following command:

```shell
./scripts/dev/cleanupLogGroups.sh
```

If you'd like to look at how the modules relate to one another in terms of their dependencies, you can run
the following development script to display the dependency structure as a graph:

```shell
./scripts/dev/showInternalDependencies.sh
```
